    ...
    <div class="row">
        ...
        <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
            ...
            <?php if ($fileType === 'upload'): ?>
                ...
                <!-- Eski Revize Talepleri (Fallback - eski sistem uyumluluğu için) -->
                <?php if (!empty($revisions)): ?>
                    <div class="detail-card mb-4">
                        <div class="detail-card-header">
                            <h5 class="mb-0">
                                <i class="fas fa-history me-2"></i>Revize Talepleri (<?php echo count($revisions); ?>)
                            </h5>
                        </div>
                        <div class="detail-card-body">
                                    ...
                                    <div class="revision-item">
                                        <div class="revision-status">
                                            ...
                                            $revisionStatusConfig = [
                                                ...
                                                'completed' => ['class' => 'info', 'text' => 'Tamamlandı', 'icon' => 'check-circle']
                                            ];
                                            $revConfig = $revisionStatusConfig[$revision['status']] ?? ['class' => 'secondary', 'text' => 'Bilinmiyor', 'icon' => 'question'];
                                            ?>
                                            <span class="badge bg-<?php echo $revConfig['class']; ?>">
                                        ...
                                        <div class="revision-content">
                                            <div class="revision-meta">
                                                <span class="revision-date">
                                                    <i class="fas fa-calendar me-1"></i>
                                                    <?php echo date('d.m.Y H:i', strtotime($revision['requested_at'])); ?>
                                                </span>
                                                <?php if (!empty($revision['admin_username'])): ?>
                                                    <span class="revision-admin">
                                                        <i class="fas fa-user-cog me-1"></i>
                                                        İncelenen: <?php echo htmlspecialchars($revision['admin_username']); ?>
                                                    </span>
                                                <?php endif; ?>
                                            </div>
                                            <div class="revision-notes">
                                                <h6>Talep Notları:</h6>
                                                <p><?php echo nl2br(htmlspecialchars($revision['request_notes'])); ?></p>
                                                <?php if (!empty($revision['admin_notes']) && $revision['status'] !== 'pending'): ?>
                                                    <h6>Admin Cevabı:</h6>
                                                    <div class="admin-response">
                                                        <?php 
                                                        if (filterAdminNotes($revision['admin_notes'])) {
                                                            echo nl2br(htmlspecialchars($revision['admin_notes']));
                                                        } else {
                                                            echo '<em class="text-muted">Admin henüz cevap vermedi.</em>';
                                                        }
                                                        ?>
                                                    </div>
                                                <?php elseif ($revision['status'] === 'pending'): ?>
                                                    <h6>Admin Cevabı:</h6>
                                                    <div class="admin-response">
                                                        <em class="text-muted">Talep henüz inceleniyor...</em>
                                                    </div>
                                                <?php endif; ?>
                                            </div>
                                        </div>
